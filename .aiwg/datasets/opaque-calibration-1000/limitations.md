...
- The worker is a deterministic calibration solver for ten project-authored task grammars. The run proves the opaque HTTP/app contract, broker, and compiler behavior in one Python process, not split-process/systemd isolation or the quality of a frontier-model teacher on open-ended material.
- The teacher created the task families and reference contracts. It did not approve records; independent review remains mandatory.
- A deterministic verifier establishes correctness only for its declared contract. It does not establish pedagogical value, naturalness, domain breadth, or absence of all subtle ambiguity.
- Exact category balance is policy-driven and should not be interpreted as an optimal production mixture.
